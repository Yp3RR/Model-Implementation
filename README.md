# ML model Implementations from scratch

Implementing core ML algorithms from scratch using numpy to understand the math and code behind it.

## ML models implemented:
- Linear Regression
- Logistic Regression
- Decision Trees
- Random Forest
- XGBoost

Main purpose of this repo is understanding the reasoning and functioning of the models. Instead of simply calling .fit() must know the why behind it.

 ## Structure

- One section per algorithm
- standalone code with system execution block

## Key concepts covered

- information gain, bootstrapping, gradient descent, etc.
- further detailed explaination for code snippets provided below respective snippets

## How to run

Simply copy paste the code and run it in terminal
- few models left to update which are knn, naive

